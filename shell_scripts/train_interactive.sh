#!/usr/bin/env bash
# train_interactive.sh — launch one SAE training job from the initial sweep.
#
# You choose only two things: the MODEL, then the TopK k. Everything else
# (hook/layer, d_sae, lr, tokens, batch, context, dtype) is fixed by the sweep
# spec below.
#
...
# = round(0.67*34) = 23; Llama is fixed at blocks.20. Architecture is TopK.
set -euo pipefail
...
hr; echo -e "${B}  SAE Sweep  —  TopK on resid_post${R}"; hr
...
printf "  %-18s %s\n" "Hook:"      "${HOOK}  (layer ${HOOK_LAYER} of ${N_LAYERS})"
printf "  %-18s %s\n" "Arch:"      "topk  k=${K}  d_sae=${D_SAE} (d_in=${D_IN} × $(( D_SAE / D_IN ))x)"
...
# Group this run's layer(s) together in W&B (Python uses the same scheme).
...
# ── Launch ────────────────────────────────────────────────────────────────────
LOG="${RUN_DIR}/logs/layer${HOOK_LAYER}.txt"
